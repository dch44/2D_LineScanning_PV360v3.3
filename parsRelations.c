...
void NAveragesRange(void)
{
  ...
  if (PVM_NAverages > 1)
  {
    ParxRelsParShowInEditor({"PVM_MotionSupOnOff"});
  }
  else
  {
    ParxRelsParHideInEditor({"PVM_MotionSupOnOff"});
  }
...
void RecoMethModeVisPar(void)
{
  ...
  if(RecoMethMode==SWI)
  {
   ParxRelsParShowInEditor({"WeightingMode","GaussBroadening"});
   if(WeightingMode==phase_image)
     ParxRelsParHideInEditor({"MaskWeighting"});
   else
     ParxRelsParShowInEditor({"MaskWeighting"}); 
  }
  else
    ParxRelsParHideInEditor({"WeightingMode","GaussBroadening","MaskWeighting"});
